...
metadataQuants<- c(0.0, 0.25, 0.5, 0.75, 0.95, 1.0)
...
`r metadataQuantPercent[6]` | `r format(metadataQuantList[[samples2process[i]]][metadataQuantPercent[6],summVar], big.mark=",", digits=3, scientific=FALSE)` | `r format(metadataQuantCellCountsList[[samples2process[i]]][metadataQuantPercent[6],summVar], big.mark=",", digits=3, scientific=FALSE)`
...
for (ii in seq(1,15,1)) {
  ...
  heatmapList[[plotLabel]] <- DimHeatmap(preProcExptsList[[samples2process[i]]], reduction = "pca", dims = ii, cells = 500, balanced = TRUE, fast=FALSE, nfeatures = 8) +
...
```{r save_data}
file2save <- sprintf("preprocessed.%s.%s.rds", projectName, Sys.Date())
