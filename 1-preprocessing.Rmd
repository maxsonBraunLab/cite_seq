...
for (sample in samples2process) {
  ...
  p <- RidgePlot(mysoWithReg, features = c("NASP", "USP1", "TUBB4B", "HMGB2", "PCNA", "TOP2A", "MCM6", "MKI67"), group.by = "Phase", ncol = 4)
  regressionPlots[[sample]][["ridgePlotCC"]] <- p

  #plot(p + plot_annotation(title = sprintf("Ridge Plot of Cell Cycle Genes in %s", sample)))

  # Plot PCAs Pre- and Post- Cell Cycle Regression
  p <- DimPlot(mysoNoReg, reduction = "pca", group.by = "Phase", split.by = "Phase")
  regressionPlots[[sample]][['pcaPreCCR']] <- p

  p <- DimPlot(mysoWithReg, reduction = "pca", group.by = "Phase", split.by = "Phase")
  regressionPlots[[sample]][['pcaPostCCR']] <- p

  #plot(p1 + p2 + plot_annotation(title = sprintf("PCA Pre- and Post- Cell Cycle Regression in %s", sample)))

  # Save Seurat Object with cell cycle regression if indicated above
  if (cellCycleRegression){

    # Look at the Features that identify the top Principle Components
    #DimHeatmap(mysoWithReg, reduction = "pca", dims = 1:15, cells = 500, balanced = TRUE)

    p <- DimHeatmap(mysoWithReg, reduction = "pca", dims = 1:15, cells = 500, balanced = TRUE)
    regressionPlots[[sample]][['heatmapPCA']] <- p

    preprocExptsList[[sample]] <- mysoWithReg
  } else{

    # Look at the Features that identify the top Principle Components
    #DimHeatmap(mysoNoReg, reduction = "pca", dims = 1:15, cells = 500, balanced = TRUE)

    p <- DimHeatmap(mysoWithReg, reduction = "pca", dims = 1:15, cells = 500, balanced = TRUE)
    regressionPlots[[sample]] <- p
...
###### `r toupper(samples2process[1])`

``` {r}
plot(regressionPlots[[samples2process[1]]][["ridgePlotCC"]] + plot_annotation(title ="Ridge Plot of Cell Cycle Genes"))
plot(regressionPlots[[samples2process[1]]][["pcaPreCCR"]] + regressionPlots[[samples2process[1]]][["pcaPostCCR"]] + plot_annotation(title = "PCA Pre- and Post- Cell Cycle Regression"))
DimHeatmap(preprocExptsList[[samples2process[1]]], reduction = "pca", dims = 1:15, cells = 500, balanced = TRUE)
```

###### `r toupper(samples2process[2])`

``` {r}
plot(regressionPlots[[samples2process[2]]][["ridgePlotCC"]] + plot_annotation(title ="Ridge Plot of Cell Cycle Genes"))
plot(regressionPlots[[samples2process[2]]][["pcaPreCCR"]] + regressionPlots[[samples2process[2]]][["pcaPostCCR"]] + plot_annotation(title = "PCA Pre- and Post- Cell Cycle Regression"))
DimHeatmap(preprocExptsList[[samples2process[2]]], reduction = "pca", dims = 1:15, cells = 500, balanced = TRUE)
```

###### `r toupper(samples2process[3])`

``` {r}
plot(regressionPlots[[samples2process[3]]][["ridgePlotCC"]] + plot_annotation(title ="Ridge Plot of Cell Cycle Genes"))
plot(regressionPlots[[samples2process[3]]][["pcaPreCCR"]] + regressionPlots[[samples2process[3]]][["pcaPostCCR"]] + plot_annotation(title = "PCA Pre- and Post- Cell Cycle Regression"))
DimHeatmap(preprocExptsList[[samples2process[3]]], reduction = "pca", dims = 1:15, cells = 500, balanced = TRUE)
```

###### `r toupper(samples2process[4])`

``` {r}
plot(regressionPlots[[samples2process[4]]][["ridgePlotCC"]] + plot_annotation(title ="Ridge Plot of Cell Cycle Genes"))
plot(regressionPlots[[samples2process[4]]][["pcaPreCCR"]] + regressionPlots[[samples2process[4]]][["pcaPostCCR"]] + plot_annotation(title = "PCA Pre- and Post- Cell Cycle Regression"))
DimHeatmap(preprocExptsList[[samples2process[4]]], reduction = "pca", dims = 1:15, cells = 500, balanced = TRUE)
```
