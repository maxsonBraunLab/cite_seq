...
# stderr and stdout to log file
f <- file("logs/1-preprocessing.log", "w")
...
for (sample_folder in strsplit(params$input_samples, " ")[[1]]) {
  if (!dir.exists(sample_folder)) {
    print(paste("ERROR: could not locate folder for sample:", sample))
    print(paste("ERROR: your sample input folder:", sample_folder))
    print(paste("ERROR: "))
...
save(list=ls(), file="mylocals.Rda") # load() this file
```
...
adt.counts.plots <- lapply(names(exptsList), function(x) {
  myso <- exptsList[[x]]
  metadata.adt <- merge(t(as.matrix(myso@assays$ADT@counts)), myso@meta.data, by = 0)
  metadata.adt <- metadata.adt %>% 
    rename("cell" = "Row.names") %>% 
    select(cell, matches("*ab-*"))
  ...
  p <- ggplot(metadata.adt.melt, aes(x = variable, y = log(value + 1), fill = variable, color = variable)) +
    ...
    ggtitle("Raw Counts per Antibody") +
