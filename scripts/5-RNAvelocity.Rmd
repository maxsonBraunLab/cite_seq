...
for (clust in sort(unique_clusters)){	
	cat("\n\n## Cluster: ",clust,"\n\n")
	cat(sprintf("Wilcoxon rank sum test for RNA expression and Velocity:   %s vs rest \n",base_line))
	...
	if (file.exists(DE_path)){
		...
		df_marker[,which(unlist(lapply(df_marker, function(x) !any(is.na(x)))))]##remove any NA
