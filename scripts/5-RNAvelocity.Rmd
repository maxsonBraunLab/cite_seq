...
unique_names <- unique(seuratObj[["orig.ident"]])
unique_names <- subset(unique_names, orig.ident %in% samples_seurat)
unique_numbs <- gsub(".*_","",rownames(unique_names))

seuratObj <- subset(seuratObj, cells = Cells(seuratObj)[which(endsWith(Cells(seuratObj), unique_numbs))])
